...
{:else}
	<input
		...
		data-slot={dataSlot}
		class={cn(
			"h-12 w-full min-w-0 rounded-xl border border-md-outline-variant bg-md-surface-container-lowest px-4 py-2 text-base text-foreground transition-all duration-200 placeholder:text-md-on-surface-variant/60 focus-visible:border-primary focus-visible:ring-3 focus-visible:ring-primary/20 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-38 md:text-sm outline-none",
			className
		)}
		{type}
		bind:value
